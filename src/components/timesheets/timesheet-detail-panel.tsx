...
import { Check, X, Clock, FileText, User, Receipt } from 'lucide-react'
...
import { useTimesheetCalculations } from './use-timesheet-calculations'
import { GranularApprovalSection } from './timesheet-approval-dialog'
import { TimesheetRejectionDialog } from './timesheet-rejection-dialog'
...
export function TimesheetDetailPanel({
  ...
  const { entries, dailyTotals, grandTotal, user, weekRange } = useTimesheetCalculations(data, weekStart, locale)
  ...
  const handleRejectConfirm = async () => {
    if (!onRejectGranular) return
    setIsProcessing(true)
    try {
      onRejectGranular(data.id, expenseData?.id ?? null, rejectTarget, rejectReason)
